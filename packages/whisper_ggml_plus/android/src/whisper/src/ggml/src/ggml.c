...
#ifdef __ANDROID__
    // Always surface the fatal message in logcat.
    __android_log_print(ANDROID_LOG_ERROR, "GGML", "%s", message);
#endif
...
{
    return tensor->nb[0] > tensor->nb[2] &&
...
{
    return tensor->ne[0] == ggml_blck_size(tensor->type) ||
...
{
    const bool can_repeat = ggml_is_empty(a) || ((ne0 % a->ne[0] == 0) &&
                                                 (ne1 % a->ne[1] == 0) &&
                                                 (ne2 % a->ne[2] == 0) &&
                                                 (ne3 % a->ne[3] == 0));
    ...
    struct ggml_context *ctx, struct ggml_cgraph *cgraph, int i, const bool *grads_needed)
{
    ...
    case GGML_OP_ROPE:
    {
        if (src0_needs_grads)
        {
            ...
            struct ggml_tensor *rope_back = grad->ne[2] == src1->ne[0] ? ggml_rope_ext_back(ctx, grad, src1, src2, n_dims,
...
void ggml_graph_print(const struct ggml_cgraph *cgraph)
{
    ...
    for (int i = 0; i < cgraph->n_nodes; i++)
    {
        ...
        GGML_LOG_INFO(" - %3d: [ %5" PRId64 ", %5" PRId64 ", %5" PRId64 "] %16s %s\n",
                      ...
                      ggml_op_name(node->op), (node->flags & GGML_TENSOR_FLAG_PARAM) ? "x" : ggml_graph_get_grad(cgraph, node) ? "g"
                                                                                                                               : " ");
    }
